...
export default function SummaryPage() {
  ...
  if (error) return <div style={{ padding: 40, color: '#C0392B' }}>Error: {error}</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {inventory.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${inventory.length}, 1fr)`, gap: 12, marginBottom: 8 }}>
            {inventory.map((inv) => {
      ...
      ) : <div style={{ padding: 24, color: '#aaa', textAlign: 'center' }}>No sales data available</div>}
      ...
      <div style={{ fontSize: '0.72rem', color: '#aaa', marginTop: 8 }}>
        Data source: MotherDuck · Updates daily at 8:30 AM ET
      </div>
    </div>
  )
}
